...
class _OrderProgressPageState extends State<OrderProgressPage> {
  ...
  Future<void> _fetch() async {
    try {
      ...
      final storeId = (orderData?['storeId'] as int?) ?? (trackingData?['storeId'] as int?);
      if (storeId != null) {
        try {
          final sUri = Uri.parse('https://chickenkitchen.milize-lena.space/api/store/$storeId');
          final sResp = await http.get(sUri, headers: const {'Accept': 'application/json'});
          if (sResp.statusCode == 200) {
            final sMap = jsonDecode(sResp.body) as Map<String, dynamic>;
            final s = sMap['data'];
            if (s is Map<String, dynamic>) {
              storeData = s;
            } else if (s is List && s.isNotEmpty && s.first is Map<String, dynamic>) {
      ...
      setState(() {
        ...
        _lastOrder = baseForChange == null ? null : Map<String, dynamic>.from(baseForChange);
        _loading = false;
        _error = null;
      });
  ...
  Widget build(BuildContext context) {
    ...
    final status = (_tracking?['status'] as String?)?.toUpperCase() ?? (_order?['status'] as String?)?.toUpperCase() ?? 'CONFIRMED';
    final steps = _stepsForStatus(status);
    final progress = (_tracking?['progress'] as num?)?.toDouble();
    final total = (_orderDetails?['totalPrice'] ?? _order?['totalPrice'] ?? _order?['total'] ?? 0) as int;
    final createdAt = (_orderDetails?['createdAt'] ?? _order?['createdAt'] ?? '') as String;
    final pickupTime = (_orderDetails?['pickupTime'] ?? _order?['pickupTime'] ?? '') as String;
    final storeName = (_store?['name'] ?? '') as String;
    final storeAddress = (_store?['address'] ?? '') as String;
    final dishes = (_tracking?['dishes'] as List<dynamic>?)?.cast<Map<String, dynamic>>() ?? const <Map<String, dynamic>>[];
    final totalCal = dishes.fold<int>(0, (sum, d) => sum + ((d['cal'] ?? 0) as int));
    final totalFromDishes = dishes.fold<int>(0, (sum, d) => sum + ((d['price'] ?? 0) as int));
              ...
              : Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text('Order #${widget.orderId}', style: const TextStyle(fontWeight: FontWeight.w700)),
                                  ...
                                  if (progress != null) ...[
                                    ...
                                    Text('${progress.toInt()}%', style: const TextStyle(fontSize: 12, color: Colors.black54)),
                                  ],
                                ],
                              ),
                            ),
                      ...
                      if (storeName.isNotEmpty || storeAddress.isNotEmpty || createdAt.isNotEmpty)
                        Container(
                          padding: const EdgeInsets.all(12),
                          decoration: BoxDecoration(
                            color: Colors.white,
                          ...
                          child: Column(
                            ...
                            children: [
                              if (storeName.isNotEmpty) ...[
                                Row(
                                  children: [
                                    ...
                                    Expanded(child: Text(storeName, style: const TextStyle(fontWeight: FontWeight.w600))),
                                  ],
                                ),
                                if (storeAddress.isNotEmpty)
                                  Padding(
                                    padding: const EdgeInsets.only(left: 22.0, top: 2),
                                    child: Text(storeAddress, style: const TextStyle(fontSize: 12, color: Colors.black54)),
                                  ),
                                const SizedBox(height: 8),
                              ],
                              Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: Text(
                                      createdAt.isEmpty ? 'Created: —' : 'Created: ${_formatDate(createdAt)}',
                              ...
                              Row(
                                children: [
                                  const Icon(Icons.payments, size: 16, color: Colors.black45),
                                  const SizedBox(width: 6),
                                  Text('Total: ${_formatVnd(displayTotal)}', style: const TextStyle(fontWeight: FontWeight.w600)),
                                  if (totalCal > 0) ...[
                                    const SizedBox(width: 16),
                                    const Icon(Icons.local_fire_department, size: 16, color: Colors.black45),
                                    const SizedBox(width: 6),
                                    Text('Calories: ${totalCal} kcal', style: const TextStyle(fontWeight: FontWeight.w600)),
                                  ],
                                ],
                              ),
                            ],
                        ...
                        child: ListView(
                          children: [
                                      ...
                                      child: Column(
                                        children: [
                                          Icon(
                                            ...
                                            color: done ? primary : Colors.black26,
                                            size: 20,
                                          ),
                                          if (i < steps.length - 1)
                                            Container(
                                              width: 2,
                                              height: 36,
                                              color: done ? primary.withOpacity(.4) : Colors.black12,
                                            ),
                                        ],
                                      ),
                                    ),
                                    Expanded(
                                      child: Container(
                                        ...
                                        decoration: BoxDecoration(
                                          color: Colors.white,
                                          borderRadius: BorderRadius.circular(12),
                                          border: Border.all(color: done ? primary.withOpacity(.4) : Colors.black12),
                                        ),
                                        child: Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                            ...
                                            const SizedBox(height: 4),
                                            Text(s.subtitle, style: const TextStyle(color: Colors.black54, fontSize: 12)),
                                          ],
                                        ),
                                      ),
                                    ),
                            ...
                            if (dishes.isNotEmpty)
                              const Text('Dishes', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                            ...
                            ...dishes.map((d) {
                              ...
                              final stepsArr = (d['steps'] as List<dynamic>?)?.cast<Map<String, dynamic>>() ?? const <Map<String, dynamic>>[];
                              return Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        const Icon(Icons.rice_bowl, color: Colors.black54),
                                        const SizedBox(width: 8),
                                        Expanded(
                                          child: Text('Dish #$dishId', style: const TextStyle(fontWeight: FontWeight.w700)),
                                        ),
                                        Text(_formatVnd(dishPrice), style: const TextStyle(fontWeight: FontWeight.w700)),
                                      ],
                                    ),
                                    const SizedBox(height: 6),
                                    Row(
                                      children: [
                                        const Icon(Icons.local_fire_department, size: 14, color: Colors.black45),
                                        const SizedBox(width: 4),
                                        Text('$dishCal kcal', style: const TextStyle(fontSize: 12, color: Colors.black54)),
                                        if (updatedAt.isNotEmpty) ...[
                                          const SizedBox(width: 12),
                                          const Icon(Icons.schedule, size: 14, color: Colors.black45),
                                          const SizedBox(width: 4),
                                          Text(_formatDate(updatedAt), style: const TextStyle(fontSize: 12, color: Colors.black54)),
                                        ]
                                      ],
                                    ),
                                    if (note.isNotEmpty) ...[
                                      const SizedBox(height: 6),
                                      Text('Note: $note', style: const TextStyle(fontSize: 12, fontStyle: FontStyle.italic, color: Colors.black87)),
                                    ],
                                    const SizedBox(height: 10),
                                    ...stepsArr.map((s) {
                                      final name = (s['stepName'] ?? 'Step') as String;
                                      final items = (s['items'] as List<dynamic>?)?.cast<Map<String, dynamic>>() ?? const <Map<String, dynamic>>[];
                                      return Padding(
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            const SizedBox(height: 6),
                                            ...items.map((it) {
                                              final itemName = (it['menuItemName'] ?? 'Item') as String;
                                              ...
                                              return Padding(
                                                ...
                                                child: Row(
                                                  children: [
                                                    Expanded(
                                                      child: Text('x$qty $itemName'),
                                                    ),
                                                    Text('${_formatVnd(price)}  •  ${cal} kcal',
                                                        style: const TextStyle(fontSize: 12, color: Colors.black54)),
                                                  ],
                                                ),
                                              );
  ...
  bool _hasOrderChanged(Map<String, dynamic>? oldOrder, Map<String, dynamic>? newOrder) {
    ...
    final oldTotal = oldOrder['total'] ?? oldOrder['totalPrice'] ?? oldOrder['price'];
    final newTotal = newOrder['total'] ?? newOrder['totalPrice'] ?? newOrder['price'];
